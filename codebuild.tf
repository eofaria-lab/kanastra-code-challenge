...
#################### CODEBUILD TEST #############################
resource "aws_codebuild_project" "kanastra-codebuild-test-project" {
...
#################### CODEBUILD LINT #############################
resource "aws_codebuild_project" "kanastra_codebuild_lint_project" {
...
resource "aws_codebuild_project" "kanastra_codebuild_project" {
  name          = var.codebuild_project_name
  description   = "kanastra build project"
  build_timeout = "5"
  service_role  = aws_iam_role.codebuild_role.arn
  ...
  environment {
    ...
    environment_variable {
      ...
      value = aws_ecr_repository.kanastra_ecr_repo.name
    }
...
resource "aws_codebuild_project" "kanastra_codebuild_deploy_project" {
  name          = var.codebuild_deploy_project_name
  description   = "kanastra codebuild deploy project"
  build_timeout = "5"
  service_role  = aws_iam_role.codebuild_role.arn
